...
param(
    ...
    [Parameter(Mandatory = $true)]
    [string] $OriginalStandardPackage,

    [string[]] $OriginalVsNvArchives = @(),
...
function Merge-VsArchive([string] $Archive, [string] $PackageRoot, [string] $StageRoot) {
    ...
    foreach ($file in Get-ChildItem -LiteralPath $stage -File -Recurse) {
        ...
        if ($file.Name -eq 'libmvtools.dll') {
            $relativeParent = Split-Path -Parent $relative
            $relative = if ([string]::IsNullOrWhiteSpace($relativeParent)) {
                'mvtools.dll'
            } else {
                Join-Path $relativeParent 'mvtools.dll'
            }
        }
...
function Add-OriginalStandardModels([string] $PackageRoot, [string] $Archive, [string] $StageRoot) {
    $stage = Join-Path $StageRoot 'original-standard-models'
    Ensure-Directory $stage
    $members = @(
        'mpv-lazy/vs-plugins/models/rife_v2/rife_v4.6.onnx',
        'mpv-lazy/vs-plugins/models/drba/distilDRBA_v2_lite_scale_ap_fp16.onnx',
        'mpv-lazy/vs-plugins/models/the_database_AnimeJaNaiV2L1_x2_fp16_op17.onnx'
    )
    Invoke-7Zip (@('e', '-y', "-o$stage", $Archive) + $members)
    ...
    foreach ($relativePath in @(
        ...
        Copy-RequiredFile (Join-Path $stage (Split-Path -Leaf $relativePath)) (Join-Path $PackageRoot "vs-plugins/models/$relativePath")
    }
}

function Merge-OriginalVsNvArchive([string] $Archive, [string] $PackageRoot) {
    # The original vsNV archive is a full overlay. Extract only runtime/plugin
    # trees directly into the package so its old Python installation and
    # portable_config cannot overwrite the freshly assembled standard package.
    $packageParent = Split-Path -Parent $PackageRoot
    Invoke-7Zip @(
        'x', '-y', "-o$packageParent", $Archive,
        'mpv-lazy/vs-plugins/*',
        'mpv-lazy/vs-scripts/*',
        'mpv-lazy/vs-coreplugins/*',
        'mpv-lazy/vsgenstubs4/*'
    )

    if (-not (Test-Path -LiteralPath (Join-Path $PackageRoot 'vs-plugins') -PathType Container)) {
        throw "The original vsNV archive did not contain any runtime overlay files: $Archive"
    }
}

function Add-VsNvConfiguration([string] $PackageRoot, [string] $StandardRoot) {
    $sourceVs = Join-Path $SourceRoot 'portable_config/vs'
    $targetVs = Join-Path $PackageRoot 'portable_config/vs'
    if (-not (Test-Path -LiteralPath $sourceVs -PathType Container)) {
        throw "The source portable_config/vs directory is missing: $sourceVs"
    }
    Copy-DirectoryContents $sourceVs $targetVs

    # The overlay is extracted on top of the standard package. Keep the same
    # model/script baseline in the standalone overlay so it is also valid when
    # inspected before extraction.
    foreach ($relativePath in @('vs-plugins/models', 'vs-scripts')) {
        $sourcePath = Join-Path $StandardRoot $relativePath
        if (Test-Path -LiteralPath $sourcePath -PathType Container) {
            Copy-DirectoryContents $sourcePath (Join-Path $PackageRoot $relativePath)
        }
    }
}
...
$OriginalStandardPackage = Resolve-FullPath $OriginalStandardPackage
$OriginalVsNvArchives = @($OriginalVsNvArchives | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | ForEach-Object { Resolve-FullPath $_ })
...
if (-not (Test-Path -LiteralPath $OriginalStandardPackage -PathType Leaf)) {
    throw "Original standard package does not exist: $OriginalStandardPackage"
}
if ($IncludeVsNv -and $VsNvArchives.Count -eq 0) {
    if ($OriginalVsNvArchives.Count -eq 0) {
        throw 'IncludeVsNv was requested, but no VS NV archives were supplied.'
    }
}
if ($IncludeVsNv) {
    $vsNvPrimaryArchives = @($VsNvArchives | Where-Object { (Split-Path -Leaf $_) -match '\.7z$|\.7z\.001$' })
    $originalVsNvPrimaryArchives = @($OriginalVsNvArchives | Where-Object { (Split-Path -Leaf $_) -match '\.7z\.001$' })
    if ($vsNvPrimaryArchives.Count -eq 0 -and $originalVsNvPrimaryArchives.Count -eq 0) {
        throw 'IncludeVsNv was requested, but no first VS NV archive volume was supplied.'
    }
}
...
Add-OriginalStandardModels $standardRoot $OriginalStandardPackage $workDirectory
...
if ($IncludeVsNv) {
    ...
    Add-VsNvConfiguration $vsNvRoot $standardRoot
    if ($originalVsNvPrimaryArchives.Count -gt 0) {
        Merge-OriginalVsNvArchive $originalVsNvPrimaryArchives[0] $vsNvRoot
    } else {
        foreach ($archive in $vsNvPrimaryArchives) {
...
$metadata = @(
    ...
    "original_standard_package=$(Split-Path -Leaf $OriginalStandardPackage)",
    "vs_cpu_archives=$($VsCpuArchives.Count)",
    "vs_nv_volumes=$($OriginalVsNvArchives.Count + $VsNvArchives.Count)",
